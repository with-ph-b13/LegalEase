  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <Link href="/dashboard/user/hiring-history" className="btn btn-outline btn-sm">
          View Hiring History
        </Link>
      </div>
              ...
              <tbody>
                {transactions.map((tx) => (
                  <tr key={tx.id || tx._id} className="hover:bg-base-200/30">
                    ...
                    <td>
                      <span className="font-medium text-secondary">Legal Consultation Fee</span>
                    </td>
                    <td>
                      {tx.lawyer ? (
                        <div>
                          <div className="font-bold">{tx.lawyer.name}</div>
                          <div className="text-sm opacity-60">{tx.lawyer.email}</div>
                        </div>
                      ) : (
                        <span className="italic opacity-50">Unknown Lawyer</span>
                      )}
                    </td>
